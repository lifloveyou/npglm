...
\begin{algorithm}[t]
	\small
	\SetAlgoLined
	\KwIn{$x_{N\times d}$ as $d$-dimensional feature vectors, $y_{N\times1}$ as observation states, and $t_{N\times1}$ as recorded times.}
	\KwOut{Learned parameters $w_{d\times1}$ and $H_{N\times1}$.}
	...
	\While{Not $converged$}{
		$\tau\leftarrow\tau+1$\;
		Use Eq.~\ref{eq:H} to obtain $H^{(\tau)}$ using $w^{(\tau-1)}$\;
		Optimize Eq.~\ref{eq:nlw} to obtain $w^{(\tau)}$ using $H^{(\tau)}$\;
		Use Eq.~\ref{eq:logl} to obtain $\log\mathcal{L}^{(\tau)}$ using $w^{(\tau)}$ and $H^{(\tau)}$\;
...
\descr{Ranged Probability.} What is the probability for the relationship $R$ to be formed between time $t_\alpha$ and $t_\beta$? This is equivalent to calculating $P(t_\alpha \le T \le t_\beta \mid x_R)$, which by definition is:
