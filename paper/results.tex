...
\begin{algorithm}[t]
	...
	\KwOut{Synthetically generated data $x_{N\times d}$ and $t_{N\times1}$.}
	$N\leftarrow N_o+N_c$\;
	...
	\For{$i\leftarrow1$ to $N$}{
		Draw feature vector $x_i\sim\mathcal{N}(0,I_d)$\;
		Set distribution parameter $\alpha_i\leftarrow\exp(w^Tx_i+b)$\;
		\uIf{$dist == Rayleigh$}{
